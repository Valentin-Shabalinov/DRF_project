...
class CourseDetailSerializer(serializers.ModelSerializer):
    ...
    def get_this_course_lessons(self, course):
        return [
            lesson.title
            for lesson in Lesson.objects.filter(course_lesson=course)
        ]
...
class LessonListSerializer(serializers.ModelSerializer):
    course_lesson = SlugRelatedField(
        slug_field="title", queryset=Course.objects.all()
    )
...
class PaymentListSerializer(serializers.ModelSerializer):
    ...
    paid_course = SlugRelatedField(
        slug_field="title", queryset=Course.objects.all()
    )
    paid_lesson = SlugRelatedField(
        slug_field="title", queryset=Lesson.objects.all()
    )
...
class PaymentCreateSerializer(serializers.ModelSerializer):
    def create(self, validated_data):
        ...
        validated_data["payment_intent_id"] = create_payment(
            int(validated_data.get("payment_amount"))
        )
